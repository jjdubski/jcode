...
interface ProcessorContext extends Input {
  ...
  // indicates the model or a tool made progress). pendingTools counts in-flight
  // tool executions (between tool-call and tool-result/error) so the timeout
  // pauses while tools run — a counter, not a boolean, because parallel tool
  // calls finish independently and a single result must not resume the watchdog
  // while another tool is still executing.
  lastActivity: Ref.Ref<number>
  pendingTools: Ref.Ref<number>
}
  ...
  Effect.gen(function* () {
    ...
    const create = Effect.fn("SessionProcessor.create")(function* (input: Input) {
      ...
      const handleEvent = Effect.fnUntraced(function* (value: StreamEvent) {
        ...
        switch (value.type) {
          ...
          case "tool-call": {
            ...
            yield* Ref.update(ctx.pendingTools, (n) => n + 1)
            yield* ensureToolCall(value)
          ...
          case "tool-result": {
            // Tool execution finished; resume the watchdog once no tools remain.
            yield* Ref.update(ctx.pendingTools, (n) => Math.max(0, n - 1))
          ...
          case "tool-error": {
            // Tool execution failed; resume the watchdog once no tools remain.
            yield* Ref.update(ctx.pendingTools, (n) => Math.max(0, n - 1))
      ...
      const process = Effect.fn("SessionProcessor.process")(function* (streamInput: LLM.StreamInput) {
        ...
        ctx.pendingTools = yield* Ref.make(0)
        ...
        return yield* Effect.gen(function* () {
          ...
          for (let i = 0; i < models.length; i++) {
            ...
            const exit = yield* Effect.gen(function* () {
              ...
              // no output for STREAM_TIMEOUT_MS while no tool is executing and no
              // user input is pending. Tool execution (e.g. the task tool waiting
              // on a subagent) pauses the watchdog via ctx.pendingTools, and
              // permission/question prompts pause it while the user decides —
              // neither counts against the model response timeout.
              const watchdog = Effect.gen(function* () {
                while (true) {
                  yield* Effect.sleep("250 millis")
                  if ((yield* Ref.get(ctx.pendingTools)) > 0) continue
                  if (yield* waitingForUser()) continue
              ...
              // True when this session has an unanswered permission request or
              // question. The processor is blocked on these until the user
              // replies (Allow/Always/Reject or a question answer), so the
              // watchdog must not count that wait toward the response timeout.
              const waitingForUser = Effect.fnUntraced(function* () {
                const permissions = yield* permission.list()
                if (permissions.some((request) => request.sessionID === ctx.sessionID)) return true
                const questions = yield* question.list()
                return questions.some((request) => request.sessionID === ctx.sessionID)
              })
